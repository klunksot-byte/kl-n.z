...
gui.Name = "TSB_Pure_Farm"
gui.ResetOnSpawn = false -- Đã fix lỗi mất khi chết
local btn = Instance.new("TextButton", gui)
btn.Size, btn.Position = UDim2.new(0, 50, 0, 50), UDim2.new(0.02, 0, 0.4, 0)
btn.BackgroundColor3 = Color3.fromRGB(40, 40, 40); btn.Text = "START"; btn.TextColor3 = Color3.new(1,1,1)
btn.Font = Enum.Font.GothamBold; btn.TextSize = 12
Instance.new("UICorner", btn).CornerRadius = UDim.new(1, 0); btn.Draggable = true

local activeEscape1 = true 

btn.MouseButton1Click:Connect(function()
    activeFarm = not activeFarm
    btn.Text = activeFarm and "STOP" or "START"
    btn.BackgroundColor3 = activeFarm and Color3.fromRGB(150, 0, 0) or Color3.fromRGB(40, 40, 40)
    activeEscape1 = not activeFarm -- Logic bật Farm tắt Escape, dừng Farm bật lại
end)
...
meleeBtn.MouseButton1Click:Connect(function()
    if activeCombat2 then return end
...
createBtn1("⚡ 10% GRAPHICS + XOÁ CÂY", Color3.fromRGB(130, 50, 0), function()
    task.spawn(function()
        ...
        for _, v in pairs(workspace:GetDescendants()) do
            ...
            if string.find(nameLower, "tree") or string.find(nameLower, "leaves") or string.find(nameLower, "leaf") or string.find(nameLower, "bush") then v:Destroy()
...
local espBtn = createBtn1("ESP LOW HP: ON", Color3.fromRGB(200, 0, 0), function(b)
    activeESP = not activeESP; b.Text = activeESP and "ESP LOW HP: ON" or "ESP LOW HP: OFF"
...
createBtn1("AUTO ESCAPE: ON", Color3.fromRGB(0,150,255), function(b)
    activeEscape1 = not activeEscape1; b.Text = activeEscape1 and "AUTO ESCAPE: ON" or "AUTO ESCAPE: OFF"
...
RS.Heartbeat:Connect(function(dt)
    if activeFarm then
        local target = getWeakestDummy()
        if target then
            Camera.CFrame = CFrame.lookAt(Camera.CFrame.Position, target.Position)
            local char = p.Character
            if char and char:FindFirstChild("HumanoidRootPart") then
                char:PivotTo(target.CFrame * CFrame.new(0, 0, 2.0))
            end
        end
    end
        ...
        if v ~= p and v.Character then
            ...
            if v.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if activeCombat2 or activeMelee then
                    targetRoot.Size = Vector3.new(_HITBOX_SIZE, _HITBOX_SIZE, _HITBOX_SIZE); targetRoot.Transparency = 0.8; targetRoot.BrickColor = BrickColor.new("Bright red"); targetRoot.Material = Enum.Material.ForceField; targetRoot.CanCollide = false
                else targetRoot.Size = Vector3.new(2, 2, 1); targetRoot.Transparency = 1 end
            end
        end
    end
    ...
    if target then
        ...
        local lookPos = target.Position; root.CFrame = CFrame.lookAt(root.Position, Vector3.new(lookPos.X, root.Position.Y, lookPos.Z)); Camera.CFrame = CFrame.new(Camera.CFrame.Position, lookPos); sInfo.Text = "TARGET: " .. target.Parent.Name
    else if hum.MoveDirection.Magnitude > 0 then root.CFrame = root.CFrame + (hum.MoveDirection * (_S * dt)) end; sInfo.Text = activeMelee and "RAGE: SCANNING..." or "STATUS: IDLE" end
end)
